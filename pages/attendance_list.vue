...
export default {
  ...
  methods: {
    ...
    getAttendanceData: function() {
      ...
      axios.get('http://localhost:8081/api/attendance/list?userId=' + this.user_id + '&date=' + month).then(response => this.attendance_list = response.data)
    },
    getAttendanceDateForDay: function(date) {
      axios.get('http://localhost:8081/api/attendance/find?userId=' + this.user_id + '&date=' + date).then(response =>{
    ...
    registAttendance: function(aproval_kind = '未申請') {
      ...
      axios.post('http://localhost:8081/api/attendance/regist', data).then(response => {
        this.dialog = false;
        let month = this.getQueryMonth();
        axios.get('http://localhost:8081/api/attendance/list?userId=' + this.user_id + '&date=' + month).then(response => this.attendance_list = response.data)
      }).catch(error => {
    ...
    cancelApplication: function(date) {
      if(confirm('申請を取り消しますか')) {
        ...
        axios.post('http://localhost:8081/api/attendance/update/status', data).then(response => {
          let month = this.getQueryMonth();
          axios.get('http://localhost:8081/api/attendance/list?userId=' + this.user_id + '&date=' + month).then(response => this.attendance_list = response.data)
        }).catch(error => {
          alert('error');
    ...
    batchApplication: function() {
      ...
      axios.post('http://localhost:8081/api/attendance/update/batch', data).then(response => {
